            ...
            <li className="inline-flex items-center truncate" key={item?.route}>
              {
                routes?.length === index + 1
                  ? <span href={item.route} className="inline-flex items-center gap-1 text-sm font-medium text-blue-gray-800">
                  {
                    index === 0 ? item.icon : BreadcrumbNext
                  }
                  {item.label}
                </span>
                  : <Link href={item.route} className="inline-flex items-center gap-1 text-sm font-medium text-gray-600 hover:text-blue-gray-600 dark:text-gray-400 dark:hover:text-white">
                  {
                    index === 0 ? item.icon : BreadcrumbNext
                  }
                  {item.label}
