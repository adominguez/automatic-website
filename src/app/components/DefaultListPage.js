  ...
  return (
    <section className='relative flex flex-col px-6 py-2 text-center text-gray-700 bg-white bg-clip-border rounded-xl'>
      ...
      <div className='py-4'>
        ...
        {
          response?.data?.length ? <div className='grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4'>
            {
              response.data?.map((item, key) => (
                hasChildren('item') ? composeItemChildren(item, key) :
        ...
        {
          isEmpty ? hasChildren('empty') ?? <div className='flex flex-col items-center justify-center p-20'>
            <div className='w-32'>
              <EmptyData />
            </div>
